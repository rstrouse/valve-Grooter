...
export class SendRecieveBuffer {
    ...
    private procTimer: NodeJS.Timer;
    ...
    public pushIn(pkt) {
        if (conn.buffer._inBuffer.length > 1) console.log(conn.buffer._inBuffer.length);
    ...
    private writeMessage(msg: Outbound) {
        ...
        if (conn.isOpen) {
            if (msg.remainingTries <= 0) {
                ...
                logger.warn(`Message aborted after ${msg.tries} attempt(s): ${msg.toPkt()}`);
                let err = new OutboundMessageError(msg, `Message aborted after ${msg.tries} attempt(s): ${msg.toPkt()}`);
